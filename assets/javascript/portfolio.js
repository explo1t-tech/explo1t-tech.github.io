...
const ipgeolocation = 'https://api.ipgeolocation.io/ipgeo?apiKey=c8c9b7ffb4014840a2a883b65c5fc4aa';
...
$(document).ready(() => {
  const links = [
    {
      name: 'Sefek - owner',
      link: '76561198257667962',
    },
    {
      name: 'Dutch',
      link: '00000000000000001',
    },
    {
      name: 'Infinite',
      link: '00000000000000001',
  ...
  for (let i in links) {
    ...
    if (i != links.length - 1) $('#marquee').append(' <span>//</span> ');
  }
  ...
  app.iconChanger(['assets/icons/favicon.png']);
});
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[unknown]</span>..."], 30, () => {
    ...
    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
      ...
      writeLine([`<i style='color: #F62459'>Exploit.tech - Feel The Kills</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
